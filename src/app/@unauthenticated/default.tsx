...
import { isFirebaseAuthenticated, isSpotifyAuthenticated } from "@/api/checkIsAuthenticated";
...
export default function unauthenticated() {
  ...
  const sendLink = () => {
    ...
    sendSignInLinkToEmail(Auth, userEmail, { url: "http://localhost:3000/redirectFirebase", handleCodeInApp: true })
  ...
  useEffect(() => {
    const handleChange = () => {
      let key = localStorage.getItem("firebaseAuthentication") as string;
      let email = localStorage.getItem("firebaseAuthEmail") as string;
      if (key && email) signInWithEmailLink(Auth, email, key)
  ...
  return (
    <div>
      ...
      <p>
        ...
        to authenticate spotify.<span>{spotify ? " Spotify is authenticated." : ""}</span>
      </p>
